...
public class PgpDecryptionUtil {
    ...
    public byte[] decrypt4WU(String encryptedText) throws PGPException, IOException {
		...
		for(int i = 0, n = encryptedTextArray.length; i < n; i++) {
			encryptedTextBytesArray[i] = Byte.valueOf(encryptedTextArray[i]);
		}
    	
        ByteArrayInputStream encryptedIn = new ByteArrayInputStream(encryptedTextBytesArray);
        ByteArrayOutputStream clearOut = new ByteArrayOutputStream();
        decrypt(encryptedIn, clearOut);
        return clearOut.toByteArray();
    }
